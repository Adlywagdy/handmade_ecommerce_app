...
class AuthCubit extends Cubit<AuthState> {
  ...
  void login({required String email, required String password}) async {
    ...
    try {
      final AuthSession session = await authService.login(
        email: email,
        password: password,
      );
...
<<<<<<< HEAD:lib/features/auth/cubit/auth_cubit.dart
      debugPrint('[AuthCubit.login] FirebaseAuthException: ${e.code} - ${e.message}');
      if (isClosed) return;
=======
      debugPrint(
        '[AuthCubit.login] FirebaseAuthException: ${e.code} - ${e.message}',
      );
>>>>>>> main:lib/features/auth/logic/auth_cubit.dart
  ...
  void _persistSession({required String? email, required AuthSession session}) {
