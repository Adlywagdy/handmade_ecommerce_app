...
class AdminFirestoreService {
  ...
  ///////////////////// Reads (streams) /////////////////////
  Stream<List<OrderModel>> streamOrders() => _db
      .collection('orders')
      .orderBy('createdAt', descending: true)
      .snapshots()
      .map(
        (s) =>
            s.docs.map((d) => OrderModel.fromJson(d.data(), id: d.id)).toList(),
      );

  Stream<List<ProductsModel>> streamProducts() => _db
      .collection('products')
      .orderBy('createdAt', descending: true)
      .snapshots()
      .map(
        (s) => s.docs
            .map((d) => ProductsModel.fromJson(d.data(), id: d.id))
            .toList(),
      );

  Stream<List<SellerData>> streamSellers() => _db
      .collection('sellers')
      .snapshots()
      .map(
        (s) =>
            s.docs.map((d) => SellerData.fromJson(d.data(), id: d.id)).toList(),
      );

  Stream<SettingsModel> streamSettings() => _db
      .doc('settings/platform')
      .snapshots()
      .map((d) => SettingsModel.fromJson(d.data() ?? {}));
  ...
  Future<void> approveSeller(String id) async {
    ...
    batch.set(_db.collection('users').doc(id), {
      'status': 'approved',
  ...
  Future<AdminStats> fetchStats() async {
    ...
    final revenue = txSnap.docs.fold<double>(
      0,
      (acc, d) => acc + ((d.data()['amount'] as num?)?.toDouble() ?? 0),
    );
    return AdminStats(
  ...
  /////////////////////// Categories /////////////////////
  Stream<List<CategoryModel>> streamCategories() => _db
  ...
  Future<void> addCategory(CategoryModel category) async {
    ...
    data['categoryId'] = category.nameEN;
    data['createdAt'] = FieldValue.serverTimestamp();
    await _db.collection('categories').doc(category.nameEN).set(data);
  }
